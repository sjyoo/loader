package Bio::KBase::Loader::Service;
...
sub call_method {
    ...
    my $ctx = Bio::KBase::Loader::ServiceContext->new($self->{loggers}->{userlog},
...
{
    # Service Loader requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for Loader but no authentication header was passed");
	}
	...
	my $stderr = Bio::KBase::Loader::ServiceStderrWrapper->new($ctx, $get_time);
	$ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service Loader.");
    }
...
package Bio::KBase::Loader::ServiceContext;

use strict;

=head1 NAME

Bio::KBase::Loader::ServiceContext
...
package Bio::KBase::Loader::ServiceStderrWrapper;
